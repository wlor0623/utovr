...
var deviceOrientation, loopHander, playObj, enableGyro = false,
	enableIframeGyro = false,
	isSetEnableGyro = false;
var playObjArr = [];

function gyroDraw() {
	if (!enableGyro || enableIframeGyro) return;
	if (deviceOrientation) {
		for (var i = 0; i < playObjArr.length; i++) {
			var curPlayer = playObjArr[i];
			curPlayer.api_setMPMatrix(deviceOrientation.getScreenAdjustedQuaternion())
		}
	}
	loopHander = requestAnimationFrame(gyroDraw)
};

function addGyroEffect(bindPlayObj) {
	playObjArr.push(bindPlayObj);
	enableGyro = true;
	deviceOrientation && deviceOrientation.start();
	gyroDraw();
	bindPlayObj.api_walkDefaultTilt(0.5)
};

function stopGyroEffect() {
	enableGyro = false;
	cancelAnimationFrame(loopHander);
...
function uvDisableGyro() {
	for (var i = 0; i < playObjArr.length; i++) {
		var curPlayer = playObjArr[i];
		curPlayer && curPlayer.api_CloseGyro()
	}
}

function uvSetEnableIframeGyro(val) {
	enableIframeGyro = val
}

function uvSetEnableGyro() {
	if (isSetEnableGyro) return;
	isSetEnableGyro = true;
	var promise = new FULLTILT.getDeviceOrientation({
		type: "game"
	});
	promise.then(function(deviceOrientationController) {
			deviceOrientation = deviceOrientationController
		})
		.catch(function(message) {
			if (window.top == window) {
				uvDisableGyro()
			} else {
				enableIframeGyro = true
			}
		})
}

function iframeGyroDraw(Quat) {
	if (!enableIframeGyro || !enableGyro) return;
	var Q = new FULLTILT.Quaternion(Quat.x || 0, Quat.y || 0, Quat.z || 0, Quat.w || 1);
	for (var i = 0; i < playObjArr.length; i++) {
		var curPlayer = playObjArr[i];
		curPlayer.api_setMPMatrix(Q)
	}
}
